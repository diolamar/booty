...
STRATEGY_PROFILES: Dict[str, Dict[str, object]] = {
    "Aggressive": {
        "strategy_regime_enabled": True,
        "strategy_min_score": 3.20,
        "strategy_min_gap": 0.65,
        "strategy_recent_columns_required": 1,
        "strategy_regime_window": 10,
        "strategy_probability_window": 24,
        "strategy_probability_min_samples": 8,
        "strategy_min_hit_probability": 0.56,
        "strategy_min_expected_value": 0.08,
        "strategy_min_probability_edge": 0.03,
        "strategy_probability_board_weight": 0.55,
    },
    "Moderate": {
        "strategy_regime_enabled": True,
        "strategy_min_score": 3.60,
        "strategy_min_gap": 0.90,
        "strategy_recent_columns_required": 1,
        "strategy_regime_window": 12,
        "strategy_probability_window": 30,
        "strategy_probability_min_samples": 10,
        "strategy_min_hit_probability": 0.62,
        "strategy_min_expected_value": 0.18,
        "strategy_min_probability_edge": 0.06,
        "strategy_probability_board_weight": 0.45,
    },
    "Conservative": {
        "strategy_regime_enabled": True,
        "strategy_min_score": 4.00,
        "strategy_min_gap": 1.10,
        "strategy_recent_columns_required": 2,
        "strategy_regime_window": 14,
        "strategy_probability_window": 36,
        "strategy_probability_min_samples": 12,
        "strategy_min_hit_probability": 0.70,
        "strategy_min_expected_value": 0.30,
        "strategy_min_probability_edge": 0.10,
        "strategy_probability_board_weight": 0.40,
    },
}
...
class AutomationEngine:
    ...
    def _should_start_unsynced(
        ...
        return int(active_round["valid_count"]) >= self.config.total_columns
    ...
    def _resolve_startup_sync_wait(
        ...
        force_unsynced = self._should_start_unsynced(startup_state)
        if force_unsynced:
            return True
        return not preserve_sync_handoff and False
    ...
    def start_simulation(self, preserve_sync_handoff: bool = False) -> bool:
        if not self.capture_mgr.is_fully_calibrated():
            return False
        if not self.capture_mgr.validate_calibration():
            self.state.update_status("Calibration validation failed - points off-screen")
    ...
    def start_autoclick(self, preserve_sync_handoff: bool = False) -> bool:
        if not self.capture_mgr.is_fully_calibrated():
            return False
        if not self.capture_mgr.validate_calibration():
            self.state.update_status("Calibration validation failed - points off-screen")
    ...
    def start_autosim(self, preserve_sync_handoff: bool = False) -> bool:
        if not self.capture_mgr.is_fully_calibrated():
            return False
        if not self.capture_mgr.validate_calibration():
            self.state.update_status("Calibration validation failed - points off-screen")
...
class AutoClickerPro:
    ...
    def _get_strategy_profile_definition(self, profile_name: str) -> Optional[Dict[str, object]]:
        return STRATEGY_PROFILES.get(profile_name)

    def _get_current_strategy_settings(self) -> Dict[str, object]:
        return {
            "strategy_regime_enabled": self.config.strategy_regime_enabled,
            "strategy_min_score": self.config.strategy_min_score,
            "strategy_min_gap": self.config.strategy_min_gap,
            "strategy_recent_columns_required": self.config.strategy_recent_columns_required,
            "strategy_regime_window": self.config.strategy_regime_window,
            "strategy_probability_window": self.config.strategy_probability_window,
            "strategy_probability_min_samples": self.config.strategy_probability_min_samples,
            "strategy_min_hit_probability": self.config.strategy_min_hit_probability,
            "strategy_min_expected_value": self.config.strategy_min_expected_value,
            "strategy_min_probability_edge": self.config.strategy_min_probability_edge,
            "strategy_probability_board_weight": self.config.strategy_probability_board_weight,
        }

    @staticmethod
    def _strategy_settings_match(left: Dict[str, object], right: Dict[str, object]) -> bool:
    ...
    def _detect_strategy_profile_name(self) -> str:
        current = self._get_current_strategy_settings()
        for profile_name, profile_values in STRATEGY_PROFILES.items():
            if self._strategy_settings_match(current, profile_values):
                return profile_name
        return "Custom"

    def _set_strategy_vars_from_values(self, values: Dict[str, object]):
        self._strategy_profile_syncing = True
        try:
            self.strategy_regime_enabled_var.set(bool(values["strategy_regime_enabled"]))
            self.strategy_min_score_var.set(f"{float(values['strategy_min_score']):.2f}")
            self.strategy_min_gap_var.set(f"{float(values['strategy_min_gap']):.2f}")
            self.strategy_recent_columns_var.set(str(int(values["strategy_recent_columns_required"])))
            self.strategy_regime_window_var.set(str(int(values["strategy_regime_window"])))
            self.strategy_probability_window_var.set(str(int(values["strategy_probability_window"])))
            self.strategy_probability_samples_var.set(str(int(values["strategy_probability_min_samples"])))
            self.strategy_min_hit_probability_var.set(f"{float(values['strategy_min_hit_probability']):.2f}")
            self.strategy_min_expected_value_var.set(f"{float(values['strategy_min_expected_value']):.2f}")
            self.strategy_min_probability_edge_var.set(f"{float(values['strategy_min_probability_edge']):.2f}")
            self.strategy_board_weight_var.set(f"{float(values['strategy_probability_board_weight']):.2f}")
        finally:
            self._strategy_profile_syncing = False

    def _populate_strategy_vars_from_config(self):
        self._set_strategy_vars_from_values(self._get_current_strategy_settings())
        self.strategy_profile_var.set(self._detect_strategy_profile_name())
        self._refresh_strategy_summary()

    def _on_strategy_field_changed(self, *_args):
        if getattr(self, "_strategy_profile_syncing", False):
            return
        if hasattr(self, "strategy_profile_var") and self.strategy_profile_var.get() != "Custom":
            self.strategy_profile_var.set("Custom")
        self._refresh_strategy_summary()

    def _build_strategy_summary_text(self) -> str:
        return (
            f"Profile: {self.strategy_profile_var.get()} | "
            f"Hit>={self.strategy_min_hit_probability_var.get()} | "
            f"EV>={self.strategy_min_expected_value_var.get()} | "
            f"Edge>={self.strategy_min_probability_edge_var.get()} | "
            f"Samples>={self.strategy_probability_samples_var.get()} | "
            f"Window={self.strategy_probability_window_var.get()}"
        )
    ...
    def _set_strategy_fields_expanded(self, expanded: bool):
        self.strategy_fields_expanded = expanded
        if not hasattr(self, "strategy_advanced_frame"):
            return

        if expanded:
            self.strategy_advanced_frame.grid()
            self.strategy_hint_label.grid()
            self.strategy_expand_btn.config(text="Hide Fields")
        else:
            self.strategy_advanced_frame.grid_remove()
            self.strategy_hint_label.grid_remove()
            self.strategy_expand_btn.config(text="Edit Fields")

    def _toggle_strategy_fields_visibility(self):
        self._set_strategy_fields_expanded(not getattr(self, "strategy_fields_expanded", False))

    def _on_strategy_profile_selected(self, _event=None):
        selected_profile = self.strategy_profile_var.get()
        if selected_profile == "Custom":
            return
        self._apply_strategy_profile(selected_profile)

    def _apply_strategy_profile(self, profile_name: str, announce: bool = True):
        profile = self._get_strategy_profile_definition(profile_name)
        if not profile:
            return
        self._set_strategy_vars_from_values(profile)
        self._apply_strategy_settings(announce=announce, preferred_profile=profile_name)

    def _apply_strategy_settings(self, announce: bool = True, preferred_profile: Optional[str] = None):
        try:
            updated_values = {
                "strategy_regime_enabled": bool(self.strategy_regime_enabled_var.get()),
                "strategy_min_score": float(self.strategy_min_score_var.get()),
                "strategy_min_gap": float(self.strategy_min_gap_var.get()),
                "strategy_recent_columns_required": int(self.strategy_recent_columns_var.get()),
                "strategy_regime_window": int(self.strategy_regime_window_var.get()),
                "strategy_probability_window": int(self.strategy_probability_window_var.get()),
                "strategy_probability_min_samples": int(self.strategy_probability_samples_var.get()),
                "strategy_min_hit_probability": float(self.strategy_min_hit_probability_var.get()),
                "strategy_min_expected_value": float(self.strategy_min_expected_value_var.get()),
                "strategy_min_probability_edge": float(self.strategy_min_probability_edge_var.get()),
                "strategy_probability_board_weight": float(self.strategy_board_weight_var.get()),
            }
        except ValueError:
            messagebox.showerror("Invalid Strategy Settings", "Please enter valid numeric values for the strategy fields.")
            return
        ...
        try:
            ...
            messagebox.showerror("Invalid Strategy Settings", str(exc))
            return

        for field_name, field_value in updated_values.items():
            setattr(self.config, field_name, getattr(validated, field_name))

        detected_profile = preferred_profile
        if detected_profile is None or not self._get_strategy_profile_definition(detected_profile):
            detected_profile = self._detect_strategy_profile_name()
        else:
            profile_values = self._get_strategy_profile_definition(detected_profile) or {}
            if not self._strategy_settings_match(updated_values, profile_values):
                detected_profile = self._detect_strategy_profile_name()

        self._set_strategy_vars_from_values(self._get_current_strategy_settings())
        self.strategy_profile_var.set(detected_profile)
        self._refresh_strategy_summary()
        self.config_dirty = True
        self._save_config()

        if announce and hasattr(self, "status_var"):
            self.status_var.set(f"Status: Strategy settings applied ({detected_profile})")
        logger.info("Strategy settings applied: %s", detected_profile)
    ...
    def _setup_ui(self):
        ...
        self._main_canvas = tk.Canvas(self._main_container, highlightthickness=0, borderwidth=0)
        ...
        self.strategy_regime_enabled_var = tk.BooleanVar(value=self.config.strategy_regime_enabled)
        self.strategy_min_score_var = tk.StringVar(value=f"{self.config.strategy_min_score:.2f}")
        self.strategy_min_gap_var = tk.StringVar(value=f"{self.config.strategy_min_gap:.2f}")
        self.strategy_recent_columns_var = tk.StringVar(value=str(self.config.strategy_recent_columns_required))
        self.strategy_regime_window_var = tk.StringVar(value=str(self.config.strategy_regime_window))
        ...
        strategy_frame = ttk.LabelFrame(panel_parent, text="Strategy Controls", padding=(10, 6))
        strategy_frame.pack(fill="x", padx=10, pady=(5, 5))
        strategy_frame.columnconfigure(1, weight=1)
        self.strategy_fields_expanded = False

        ttk.Label(strategy_frame, text="Profile:", font=("Arial", 9, "bold")).grid(row=0, column=0, sticky="w", padx=(0, 6), pady=(0, 6))
        self.strategy_profile_combo = ttk.Combobox(
            strategy_frame,
            textvariable=self.strategy_profile_var,
            values=("Aggressive", "Moderate", "Conservative", "Custom"),
            state="readonly",
            width=14,
        )
        ...
        ttk.Button(strategy_frame, text="Apply Values", command=self._apply_strategy_settings).grid(
            row=0, column=2, sticky="w", padx=(8, 0), pady=(0, 6)
        )

        ttk.Checkbutton(
            strategy_frame,
            text="Use regime filter",
            variable=self.strategy_regime_enabled_var,
        ).grid(row=0, column=3, sticky="w", padx=(10, 0), pady=(0, 6))

        self.strategy_expand_btn = ttk.Button(
            strategy_frame,
            text="Edit Fields",
            command=self._toggle_strategy_fields_visibility,
        )
        self.strategy_expand_btn.grid(row=0, column=4, sticky="e", padx=(8, 0), pady=(0, 6))
        ...
        ttk.Label(
            ...
            wraplength=520,
            justify="left",
        ).grid(row=1, column=0, columnspan=5, sticky="w", pady=(0, 4))

        strategy_fields = [
            ("Min Score", self.strategy_min_score_var),
            ("Min Gap", self.strategy_min_gap_var),
            ("Recent Cols", self.strategy_recent_columns_var),
            ("Regime Window", self.strategy_regime_window_var),
            ("Prob Window", self.strategy_probability_window_var),
            ("Min Samples", self.strategy_probability_samples_var),
            ("Min Hit Prob", self.strategy_min_hit_probability_var),
        ...
        self.strategy_advanced_frame = ttk.Frame(strategy_frame)
        self.strategy_advanced_frame.grid(row=2, column=0, columnspan=5, sticky="ew")

        for index, (label_text, variable) in enumerate(strategy_fields):
            row = 1 + (index // 2)
            column_group = (index % 2) * 2
            ttk.Label(self.strategy_advanced_frame, text=label_text).grid(row=row, column=column_group, sticky="e", padx=(0, 6), pady=2)
            ttk.Entry(self.strategy_advanced_frame, textvariable=variable, width=8).grid(row=row, column=column_group + 1, sticky="w", pady=2)
            variable.trace_add("write", self._on_strategy_field_changed)

        self.strategy_regime_enabled_var.trace_add("write", self._on_strategy_field_changed)
        self.strategy_hint_label = ttk.Label(
            strategy_frame,
            text="Preset selection updates the fields. Manual edits switch the profile to Custom until you apply them.",
            font=("Arial", 8),
            wraplength=520,
        )
        self.strategy_hint_label.grid(row=3, column=0, columnspan=5, sticky="w", pady=(6, 0))
        self._set_strategy_fields_expanded(False)

        status_frame = ttk.LabelFrame(panel_parent, text="Status", padding=(10, 5))
        ...
        game_frame = ttk.LabelFrame(panel_parent, text="Game State", padding=(10, 5))
        ...
        stats_frame = ttk.LabelFrame(panel_parent, text="Statistics", padding=(10, 5))
        ...
        info_frame = ttk.LabelFrame(panel_parent, text="Information", padding=(10, 5))
...
"""
    ...
    def _prepare_betting_mode_handoff(self) -> bool:
        ...
        preserve_sync_handoff = (
            switching_between_betting_modes
            and self.engine._can_preserve_betting_handoff_sync()
        )

        if current_mode != AutomationState.IDLE:
            self.engine.stop_all()

        if switching_between_betting_modes and not preserve_sync_handoff:
            logger.info("Betting mode handoff requires resync before next bet")

        return preserve_sync_handoff
    ...
    def start_simulation(self):
        ...
        preserve_sync_handoff = self._prepare_betting_mode_handoff()

        self.engine._ensure_brave_on_top("start simulation")

        if self.engine.start_simulation(preserve_sync_handoff=preserve_sync_handoff):
    ...
    def start_autoclick(self):
        ...
        if self.engine.start_autoclick(preserve_sync_handoff=preserve_sync_handoff):
    ...
    def start_autosim(self):
        ...
        if self.engine.start_autosim(preserve_sync_handoff=preserve_sync_handoff):
