...
class AutoClickerPro:
    ...
    def _run_lazy_history_analysis(self, csv_path: Path, csv_mtime: float):
        try:
            model = build_history_strategy_model(csv_path)
    ...
    def _build_strategy_summary_text(self) -> str:
        ...
        return (
            f"Profile: {profile_name} | Data-driven only | "
            ...
            f"Fib@L{self.config.strategy_fibonacci_trigger_loss}"
        )
    ...
    def _apply_custom_strategy_fields(self):
        ...
        if hasattr(self, "status_var"):
            profile_name = self._detect_strategy_profile_name()
            if profile_name == "Custom":
                self.status_var.set("Status: Custom strategy applied")
            else:
                self.status_var.set(f"Status: Strategy now matches profile {profile_name}")
        logger.info(
            "Custom strategy fields applied: window=%s samples=%s hit=%.2f ev=%.2f edge=%.2f board_weight=%.2f fib_trigger=%s",
    ...
    def _setup_ui(self):
        ...
        ttk.Button(
            ...
            text="i",
            width=3,
            command=self._show_strategy_info_dialog,
        ).grid(row=0, column=4, sticky="e", padx=(8, 0), pady=(0, 6))
        ...
        self.strategy_custom_fields_frame = ttk.LabelFrame(strategy_frame, text="Custom Fields", padding=(8, 6))
        self.strategy_custom_fields_frame.grid(row=2, column=0, columnspan=5, sticky="ew", pady=(6, 0))
        for column_index in range(4):
            self.strategy_custom_fields_frame.columnconfigure(column_index, weight=1)
        ...
        for index, (label_text, variable) in enumerate(custom_fields):
            row = index // 2
            column = (index % 2) * 2
            ttk.Label(self.strategy_custom_fields_frame, text=f"{label_text}:").grid(
                row=row,
                column=column,
                sticky="w",
                padx=(0, 6),
            ...
            entry = ttk.Entry(self.strategy_custom_fields_frame, textvariable=variable, width=14)
        ...
        ttk.Button(
            self.strategy_custom_fields_frame,
            text="Apply Custom",
            command=self._apply_custom_strategy_fields,
        ).grid(row=3, column=0, columnspan=2, sticky="w", pady=(6, 0))
        ttk.Label(
            self.strategy_custom_fields_frame,
            text="Pick Custom, edit values, then apply. If values match a preset exactly, the profile label will switch to that preset.",
            font=("Arial", 8),
            justify="left",
            wraplength=620,
    ...
    def _update_ui(self):
        ...
        if (
            ...
            self.martingale_var.set(
                ...
                f"Loss streak: {self.engine.loss_streak}/{self.engine.max_loss_streak - 1}"
            )
        else:
