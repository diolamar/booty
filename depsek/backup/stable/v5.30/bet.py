    ...
    def _handle_daily_idle_cutoff(self, prefix: str, first_round: bool = False) -> bool:
        ...
        if self._is_daily_idle_due(now):
            ...
            if self.loss_streak > 0:
                ...
                self.state.update_status(
                    f"{prefix}: 03:00 mandatory idle reached during loss streak {self.loss_streak}. Extending until a win clears the streak, then status will switch to Idle."
                )
                self._save_session_state()
                return False
            self.loss_streak = 0
            ...
            self._request_idle_shutdown(
                f"{prefix}: 03:00 mandatory idle reached. Loss streak already 0; status set to Idle; manual restart required after 05:00 AM."
            )
            return True
        if self._scheduled_idle_waiting_for_win:
            if self.loss_streak == 0:
                self._request_idle_shutdown(
                    f"{prefix}: 03:00 mandatory idle extension completed after a win cleared the loss streak. Status set to Idle."
                )
                return True
            return False
        if self._soft_idle_waiting_for_win:
            if self.loss_streak <= 1:
                ...
                self.state.update_status(
                    f"{prefix}: soft idle streak extension completed, but profit is still {self.profit_total:.2f}. Extending until profit becomes positive, max loss streak hits {self.max_loss_streak}, or 03:00 mandatory idle."
                )
                self._save_session_state()
                return False
        ...
        if self.loss_streak > 1:
            self._soft_idle_waiting_for_win = True
            self.state.update_status(
                f"{prefix}: soft idle runtime {runtime_text} reached during loss streak {self.loss_streak}. Extending until a win clears the streak to 1 or lower; if profit is still negative after that, soft idle will keep extending; 03:00 mandatory idle remains active."
            )
            self._save_session_state()
            return False
        ...
        if self.profit_total > 0 and self.loss_streak <= 1:
            self._request_idle_shutdown(
                f"{prefix}: soft idle runtime {runtime_text} reached. Status set to Idle; 03:00 mandatory idle remains active."
            )
            return True

        self._scheduled_idle_waiting_for_profit = True
        self.state.update_status(
            f"{prefix}: soft idle runtime {runtime_text} reached with profit {self.profit_total:.2f}. Extending until profit becomes positive and loss streak is 1 or lower, max loss streak hits {self.max_loss_streak}, or 03:00 mandatory idle."
        )
        self._save_session_state()
        return False
...
class AutoClickerPro:
    ...
    def _build_strategy_summary_text(self) -> str:
        ...
        if self.strategy_randomize_enabled_var.get():
            return (
                ...
                f"SoftIdle={soft_idle_text} | 03:00 mandatory idle retained"
            )
        return (
    ...
    def _build_strategy_info_text(self) -> str:
        ...
        lines = [
            ...
            "- 03:00 mandatory idle remains separate and can still stop the run later.",
            "",
            "Decision Flow",
    ...
    def _is_fibonacci_trigger_edit_allowed(self) -> bool:
        return self._is_history_window_limit_edit_allowed()
    ...
    def _refresh_fibonacci_trigger_control_state(self):
        fib_combo = getattr(self, "strategy_fibonacci_trigger_combo", None)
        if not fib_combo:
            return

        if self._is_fibonacci_trigger_edit_allowed():
            fib_combo.configure(state="readonly")
        else:
            fib_combo.configure(state="disabled")
    ...
    def _refresh_strategy_custom_fields_visibility(self):
        ...
        self._refresh_fibonacci_trigger_control_state()
    ...
    def _apply_fibonacci_trigger_selection(self):
        if getattr(self, "_strategy_profile_syncing", False):
            return
        if not self._is_fibonacci_trigger_edit_allowed():
            self._populate_strategy_vars_from_config()
            if hasattr(self, "status_var"):
                self.status_var.set(
                    "Status: Fib Trigger can only be edited during warmup while Simulate, AutoClick, or AutoSim is running"
                )
            return
        try:
            candidate_data = self.config.to_dict()
        ...
        if hasattr(self, "status_var"):
            if self.config.scheduled_idle_lead_minutes <= 0:
                if preserve_active_extension:
                    ...
                    self.status_var.set("Status: Soft idle minutes disabled; only 03:00 mandatory idle remains")
            else:
                if preserve_active_extension:
                    self.status_var.set(
